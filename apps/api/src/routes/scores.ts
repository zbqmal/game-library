...
export async function scoresRoutes(fastify: FastifyInstance) {
  ...
  fastify.post<{
    ...
    async (
      ...
      // Cast request.body to Partial<CreateScorePayload> for runtime validation
      const payload = request.body as Partial<CreateScorePayload>;
      ...
      if (!payload.gameId || !payload.playerName || typeof payload.score !== 'number') {
        return {
          success: false,
